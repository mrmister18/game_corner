...
const Hangman = () => {
  ...
  function checkWin(gameState) {
    for (let i = 0; i < gameState.word.length; i++) {
      if (!alphabet.includes(gameState.word[i].toUpperCase())) {
        continue;
      }
      ...
      <div className="row hangman-board">
        <div className="column" style={{ width: "50%" }}>
          <div id="hangman-visual" className="setup column">
            <div
              id="rafter"
              className={gameState.tries >= 8 ? null : "active"}
            ></div>
            <div className="row" id="between">
              <div id="hanged-man" className="row">
                <div
                  id="left-arm"
                  style={gameState.tries >= 3 ? { borderColor: "black" } : null}
                ></div>
                <div id="center" className="column">
                  <div
                    id="rope"
                    className={gameState.tries >= 7 ? null : "active"}
                  ></div>
                  <div
                    id="head"
                    className={gameState.tries >= 6 ? null : "active"}
                  ></div>
                  <div
                    id="body"
                    className={gameState.tries >= 5 ? null : "active"}
                  ></div>
                  <div id="legs" className="row">
                    <div
                      id="left-leg"
                      className={gameState.tries >= 2 ? null : "active"}
                    ></div>
                    <div
                      id="right-leg"
                      className={gameState.tries >= 1 ? null : "active"}
                    ></div>
                  </div>
                </div>
                <div
                  id="right-arm"
                  style={gameState.tries >= 4 ? { borderColor: "black" } : null}
                ></div>
              </div>
              <div
                id="pole"
                className={gameState.tries >= 9 ? null : "active"}
              ></div>
            </div>
            <div
              id="floor"
              className={gameState.tries >= 10 ? null : "active"}
            ></div>
          </div>
        ...
        <div className="column" style={{ width: "50%", height: "50%" }}>
          <div className="row" style={{ height: "50%" }}>
                ...
                return (
                  <span
                    ...
                    className={`smaller-button ${
                      gameState.guesses.includes(letter)
                        ? gameState.word.toUpperCase().includes(letter)
                          ? "active"
                          : "incorrect"
                        : gameState.gameOver
                        ? null
            ...
            <div className="column" style={{ alignItems: "stretch" }}>
              <div
                className="button highlight clickable"
                onClick={async () => {
                  let gameStateCopy = { ...gameState };
          ...
          <div id="hint">
            {hint || gameState.gameOver ? gameState.definition : null}
          </div>
        </div>
      </div>
    </div>
